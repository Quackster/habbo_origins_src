...
on sendGameParameterValues me, tParamList, tTeamId
  ...
  repeat with i = 1 to tStruct.count
    ...
    if tValueData[#name] = "secondsUntilStart" then
      tValueData[#max] = 90
    end if
    if tValueData[#name] = "gameLengthSeconds" then
      tValueData[#max] = 150
    end if
    if tValueData[#name] = "secondsUntilRestart" then
      tValueData[#max] = 60
    end if
    if tValue = VOID then
...
on sendJoinParameterValues me, tInstanceId, tTeamId, tParamList
  ...
  tTeamId = tTeamId - 1
  tOutput = [#integer: tInstanceId, #integer: tTeamId, #integer: tParamList.count]
  repeat with i = 1 to tParamList.count
