...
on retrieveCatalogueIndex me
  ...
  if not voidp(pCatalogProps["catalogueIndex"]) and (tEditmode <> "develop") then
    ...
    if connectionExists(getVariable("connection.info.id")) then
      return getConnection(getVariable("connection.info.id")).send("GCIX", [#string: tEditmode, #string: tLanguage])
    else
      return 0
    end if
  end if
...
on retrieveCataloguePage me, tPageID
  ...
  if not voidp(pCatalogProps[tPageID]) and (tEditmode <> "develop") then
    ...
    if connectionExists(getVariable("connection.info.id")) then
      return getConnection(getVariable("connection.info.id")).send("GCAP", [#string: tEditmode, #string: tPageID, #string: tLanguage])
    else
      return 0
    end if
  end if
