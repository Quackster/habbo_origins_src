...
on handle_regok me, tMsg
  tParam = tMsg.connection.GetStrFrom(tMsg)
  getObject(#session).set("registration_totp_code", tParam)
...
on handle_submit_gdpr_response me, tMsg
  tParam = tMsg.connection.GetIntFrom(tMsg)
  me.getInterface().responseToGdprRequest(tParam)
  return 1
end

on handle_email_approved me, tMsg
  tEmailAddress = tMsg.connection.GetStrFrom(tMsg)
  me.getInterface().userEmailOk()
  me.getComponent().sendTotpEmailVerificationNeededQuery(tEmailAddress)
  return 1
end
...
on regMsgList me, tBool
  ...
  tMsgs.setaProp(1570, #handle_submit_gdpr_response)
  tMsgs.setaProp(1575, #handle_totp_email_verification_required)
  tCmds = [:]
  ...
  tCmds.setaProp("SUBMIT_GDPR_REQUEST", 1570)
  tCmds.setaProp("TOTP_EMAIL_REQUIRED", 1575)
  if tBool then
