...
on totpEmailVerificationStatus me, tFlag
  if pmode = "topt_verification" then
    if tFlag then
      me.changePage(1)
    else
      me.totpVerificationNotNeeded()
      me.changePage(1)
    end if
  end if
end
...
on openGdprRequestWindow me
  tWindowTitleStr = getText("reg_gdpr_request")
  tWndType = "reg_gdpr_requests.window"
  if not createWindow(pVerifyChangeWndID, VOID, 0, 0, #modal) then
    return 0
  end if
  tWinObj = getWindow(pVerifyChangeWndID)
...
on defineModes me, tMode
  ...
  if (pmode = "registration") or (pmode = "parent_email") or (pmode = "parent_email_strong_coppa") or (pmode = "topt_verification") then
    pNameChecked = 0
  else
    pNameChecked = 1
    me.getMyInformation()
...
on NewFigureInformation me
  ...
  pPropsToServer["onlineStatus"] = 1
  pPropsToServer["publicProfileEnabled"] = 1
  pPropsToServer["friendRequestsEnabled"] = 1
  pPropsToServer["offlineMessagingEnabled"] = 1
  pPropsToServer["totpCode"] = EMPTY
...
on getMyInformation me
  pPropsToServer = [:]
  tTempProps = ["name", "password", "figure", "sex", "customData", "email", "birthday", "directMail", "onlineStatus", "publicProfileEnabled", "friendRequestsEnabled", "offlineMessagingEnabled"]
...
on setMyDataToFields me
  ...
  if (pRegProcess <> 0) and (pRegProcessLocation.ilk = #integer) then
    ...
    if pRegProcessLocation = pRegProcess.count then
      if (pmode <> "registration") and (pmode <> " parent_email") and (pmode <> "topt_verification") then
...
on totpVerificationNotNeeded me
  if pRegProcess.ilk = #list then
    tPos = pRegProcess.findPos("reg_totp_email")
    if tPos > 0 then
      pRegProcess.deleteAt(tPos)
    end if
  end if
...
on registrationReady me
  ...
  getObject(#session).set(#email, pPropsToServer["email"])
  getObject(#session).set("user_figure", pPropsToServer["figure"].duplicate())
  if (pmode = "registration") or (pmode = "parent_email") or (pmode = "parent_email_strong_coppa") or (pmode = "topt_verification") then
...
on leavePage me, tCurrentWindow
  case tCurrentWindow of
    ...
    "reg_parent_email.window":
      ...
      me.ChangeWindowView("reg_loading.window")
      return 0
    "reg_totp_email.window":
      tWndObj = getWindow(pWindowTitle)
      tTotpCode = tWndObj.getElement("reg_totp_email").getText()
      if tTotpCode = EMPTY then
        return me.parentEmailIncorrect()
      end if
      pPropsToServer["totpCode"] = tTotpCode
      if pRegProcess.ilk = #list then
        tPos = pRegProcess.findPos("reg_totp_email")
        tNextPage = pRegProcess[tPos + 1]
        me.changePage(tNextPage & ".window")
...
on enterPage me, tWindow
  ...
  case tWindow of
    ...
    "reg_info_update.window":
      me.setMyDataToFields()
      me.updateCheckButton("char_spam_checkbox", "directMail")
      me.updateCheckButton("char_online_status_checkbox", "onlineStatus")
      me.updateCheckButton("char_friend_requests_enabled_checkbox", "friendRequestsEnabled")
      me.updateCheckButton("char_offline_messaging_enabled_checkbox", "offlineMessagingEnabled")
      me.updateCheckButton("char_public_profile_enabled_checkbox", "publicProfileEnabled")
      tWinObj = getWindow(pWindowTitle)
      ...
      tStr = tWinObj.getElement("gdpr_request").getText()
      tStr = tStr & " >>"
      tWinObj.getElement("gdpr_request").setText(tStr)
    "reg_confirm.window":
      me.updateCheckButton("char_spam_checkbox", "directMail")
      me.setMyDataToFields()
    "reg_done.window":
...
on responseToGdprRequest me, tStatus
  ...
  case tStatus of
    "1":
      executeMessage(#alert, [#msg: "Alert_GDPR_In_Progress", #id: "GDPR_In_Progress", #modal: 1])
    "3":
      executeMessage(#alert, [#msg: "Alert_GDPR_Success", #id: "GDPR_Success", #modal: 1])
    otherwise:
      tWndObj.merge("reg_gdpr_request_error.window")
  end case
end
...
on eventProcFigurecreator me, tEvent, tSprID, tParm, tWndID
  ...
  if tEvent = #mouseUp then
    case tSprID of
      ...
      "char_online_status_checkbox":
        me.updateCheckButton("char_online_status_checkbox", "onlineStatus", 1)
      "char_friend_requests_enabled_checkbox":
        me.updateCheckButton("char_friend_requests_enabled_checkbox", "friendRequestsEnabled", 1)
      "char_offline_messaging_enabled_checkbox":
        me.updateCheckButton("char_offline_messaging_enabled_checkbox", "offlineMessagingEnabled", 1)
      "char_public_profile_enabled_checkbox":
        me.updateCheckButton("char_public_profile_enabled_checkbox", "publicProfileEnabled", 1)
      "char_terms_checkbox":
          ...
          case the keyCode of
            ...
            otherwise:
              ...
              if not (tValidKeys = EMPTY) then
                ...
                if pTempPassword[tSprID].length > 32 then
...
on eventProcVerifyWindow me, tEvent, tSprID, tParm, tWndID
  ...
  if tEvent = #keyDown then
    ...
    case tSprID of
      ...
      "gdpr_access_request_button":
        tWndObj = getWindow(pVerifyChangeWndID)
        tWndObj.unmerge()
        tWndObj.merge("reg_update_progress.window")
        pLastWindow = "reg_gdpr_requests.window"
        me.blinkChecking()
        tProp = ["gdprRequestType": 0]
        me.getComponent().submitGdprRequestMsg(tProp)
      "gdpr_erasure_request_button":
        tWndObj = getWindow(pVerifyChangeWndID)
        tWndObj.unmerge()
        tWndObj.merge("reg_update_progress.window")
        pLastWindow = "reg_gdpr_requests.window"
        me.blinkChecking()
        tProp = ["gdprRequestType": 2]
        me.getComponent().submitGdprRequestMsg(tProp)
    end case
  end if
end
