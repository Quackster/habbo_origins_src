...
on construct me
  ...
  pTotpVerificationCodeNeededFlag = VOID
  pTotpVerificationCode = EMPTY
  pRegMsgStruct = [:]
  ...
  pRegMsgStruct["onlineStatus"] = [#id: 14, "type": #boolean]
  pRegMsgStruct["publicProfileEnabled"] = [#id: 15, "type": #boolean]
  pRegMsgStruct["friendRequestsEnabled"] = [#id: 16, "type": #boolean]
  pRegMsgStruct["offlineMessagingEnabled"] = [#id: 17, "type": #boolean]
  pRegMsgStruct["totpCode"] = [#id: 18, "type": #string]
...
on sendFigureUpdateToServer me, tPropList
  ...
  repeat with tProp in ["figure", "sex", "customData", "directMail", "has_read_agreement", "parentagree", "onlineStatus", "publicProfileEnabled", "friendRequestsEnabled", "offlineMessagingEnabled", "totpCode"]
    ...
    if (not [tValue].getPos(tStoredValue) and not voidp(tValue)) or not tValueExists then
      if not voidp(pRegMsgStruct[tProp]) then
        tMsg.addProp(#short, pRegMsgStruct[tProp].id)
        if pRegMsgStruct[tProp].type = #boolean then
          tValue = integer(tValue)
...
on sendTotpEmailVerificationNeededQuery me, tEmail
  if connectionExists(getVariable("connection.info.id")) then
    getConnection(getVariable("connection.info.id")).send("TOTP_EMAIL_REQUIRED", [#string: tEmail])
  end if
  return 1
end
...
on onTotpEmailVerificationNeedQueryResult me, tFlag
  pTotpVerificationCodeNeededFlag = tFlag
  return me.getInterface().totpEmailVerificationStatus(tFlag)
end
...
on submitGdprRequestMsg me, tPropList
  if not ilk(tPropList, #propList) then
    ...
    if voidp(tPropList["gdprRequestType"]) then
      return error(me, "Missing GDPR request type:" && tPropList, #sendUpdateMsg)
    end if
  end if
  tValue = tPropList["gdprRequestType"]
  if connectionExists(getVariable("connection.info.id")) then
    getConnection(getVariable("connection.info.id")).send("SUBMIT_GDPR_REQUEST", [#integer: integer(tValue)])
  end if
end
  ...
  case tstate of
    ...
    "openFigureCreator":
      ...
      if threadExists(#login) and not connectionExists(getVariable("connection.info.id")) then
        ...
        tRegistrationProcessMode = "topt_verification"
