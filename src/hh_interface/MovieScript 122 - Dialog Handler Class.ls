...
on construct me
  ...
  pReportPageNr = 1
  pChatPageNr = 1
  pReportableUsersPageCount = 1
  pReportableChatPageCount = 1
...
on buildResources me
  pWriterPlain = "dialog_writer_plain_dh"
  pWriterLink = "dialog_writer_link_dh"
  pWriterBold = "dialog_writer_bold_dh"
...
on openCfhWindowWithReportingOptions me
  ...
  pReportableUsersPageCount = pReportedUserSelectionList.count / pTotalReportableUsers
  if (pReportedUserSelectionList.count mod pTotalReportableUsers) <> 0 then
    pReportableUsersPageCount = pReportableUsersPageCount + 1
  end if
...
on updateReportView me
  ...
  tElem = tWndObj.getElement("cfh_report_user_pagecounter")
  tElem.setText(getText("cfh_pagecounter") && pReportPageNr && "/" && pReportableUsersPageCount)
  tElem = tWndObj.getElement("cfh_user_selector_prev")
  if pReportPageNr = 1 then
    tElem.hide()
  else
    tElem.show()
  end if
  tElem = tWndObj.getElement("cfh_user_selector_next")
  if pReportPageNr = pReportableUsersPageCount then
    tElem.hide()
  else
    tElem.show()
  end if
...
on nextChatPage me
  if pChatPageNr < pReportableChatPageCount then
    pChatPageNr = pChatPageNr + 1
    me.updateChatReportView()
  end if
end

on prevChatPage me
  if pChatPageNr > 1 then
    pChatPageNr = pChatPageNr - 1
    me.updateChatReportView()
  end if
end
...
on selectReportedchatLine me, tElementId
  ...
  tNum = integer(tElementId.char[tPrefixLength..tElementId.char.count])
  tStartNr = (pChatPageNr - 1) * pTotalReportableChatLines
...
on showChatSelectionWindow me
  ...
  repeat with i = 1 to pReportedUserSelectionList.count
    if pReportedUserSelectionList[i].selected then
      pReportedUser = pReportedUserSelectionList[i].name
    end if
  end repeat
...
on updateChatReportView me
  tWndObj = getWindow(pReportWindowTitle)
  tChatHistory = me.getChatHistoryForUser(pReportedUser)
  if count(tChatHistory) > 0 then
    pReportableChatPageCount = count(tChatHistory) / pTotalReportableChatLines
    tNewList = [:]
    ...
    tElem = tWndObj.getElement("cfh_report_chat_pagecounter")
    tElem.setText(getText("cfh_pagecounter") && pChatPageNr && "/" && pReportableChatPageCount)
    tElem = tWndObj.getElement("cfh_chat_select_prev")
    if pChatPageNr = 1 then
      tElem.hide()
    else
      tElem.show()
    end if
    tElem = tWndObj.getElement("cfh_chat_select_next")
    if pChatPageNr = pReportableChatPageCount then
      tElem.hide()
    else
      tElem.show()
...
on eventProcCallForHelpWithReportingWindow me, tEvent, tElemID, tParam, tWndID
  if tEvent = #mouseUp then
    case tElemID of
      "close", "callhelp_cancel", "alertsent_ok", "report_cancel_button":
        me.removeDialog(tWndID, pWindowList)
        if windowExists(tWndID) then
          tWndObj = getWindow(tWndID)
          tWndObj.close()
        end if
      "cfh_user_selector_next":
        me.nextUserPage()
      "cfh_user_selector_prev":
        me.prevUserPage()
      "cfh_chat_select_next":
        me.nextChatPage()
      "cfh_chat_select_prev":
        me.prevChatPage()
