...
on handle_vote_results me, tMsg
  ...
  tQuestion = tMsg.content.line[1]
  tChoices = []
  tTotalLines = tMsg.content.line.count
  repeat with i = 2 to tTotalLines - 1
    tLine = tMsg.content.line[i]
    if length(tLine) > 2 then
      tChoices.add(tLine.char[3..length(tLine)])
    end if
  ...
  the itemDelimiter = "/"
  tLine = tMsg.content.line[tTotalLines]
...
on handle_votes_close me, tMsg
  me.getInterface().hideVoteScreen()
end
...
on regMsgList me, tBool
  ...
  tMsgs.setaProp(82, #handle_votes_close)
  tCmds = [:]
