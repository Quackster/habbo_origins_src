...
on isVisible me
  return windowExists(pCatalogID)
end
...
on hidePurchaseOk me, tOptionalEvent, tOptionalSprID
  if tOptionalEvent = #mouseUp then
    if stringp(tOptionalSprID) then
      if (tOptionalSprID = "button_cancel") or (tOptionalSprID = "alert_ok") then
        if windowExists(pInfoWindowID) then
          removeWindow(pInfoWindowID)
        end if
      end if
      if ((tOptionalSprID <> "close") and (tOptionalSprID <> "habbo_message_ok")) or (tOptionalSprID = "button_cancel") or (tOptionalSprID = "alert_ok") then
        return 0
      end if
    end if
  end if
